...
def test(points, title=None, parent=None, test_point_id = 0):
    ...
    def register_test(fn, title=title):
        ...
        def run_test():
            ...
            sc.pending_testpoint(test_point_id)
            ...
            if fail:
                sc.reject_testpoint(test_point_id)
            else:
                sc.pass_testpoint(test_point_id)
...
def make(*target):
    if not target: # tuple is empty
        sc.begin_compile()
    pre_make()
    proc = Popen(("make",) + target, stderr = subprocess.PIPE)
    if proc.wait():
        info = proc.stderr.read()
        if not target:
            sc.compile_error(info)
        else:
            sc.judge_error(info)
        sys.exit(1)
    post_make()
...
'killall qemu' or 'killall qemu.real'.""" % self.get_gdb_port(), file=sys.stderr)
            sc.judge_error("QEMU already running")
            sys.exit(1)
    ...
    def run_qemu(self, *monitors, **kw):
        ...
        try:
            ...
            if self.gdb is None:
                ...
                sc.judge_error("QEMU connect fail")
                sys.exit(1)
            post_make()
